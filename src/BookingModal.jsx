...
function BookingModal({ onClose }) {
  ...
  const [bins, setBins] = useState([{ type: 'Green', count: 1, frequency: 'One-off' }]);
  const [address, setAddress] = useState('');
  const [email, setEmail] = useState('');

  const handleAddBin = () => {
    setBins([...bins, { type: 'Green', count: 1, frequency: 'One-off' }]);
  };
  ...
  const handleSend = () => {
    const binDetails = bins.map(b => `${b.type} x${b.count} (${b.frequency})`).join(', ');
      ...
      <div className="bg-white rounded-lg w-full max-w-md p-6">
        ...
        {bins.map((bin, idx) => (
          <div key={idx} className="mb-3">
            ...
            <select
              value={bin.frequency}
              onChange={e => handleBinChange(idx, 'frequency', e.target.value)}
        ...
        <button onClick={handleAddBin} className="text-green-600 text-sm mb-4">+ Add Another Bin</button>
